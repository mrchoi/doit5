...
<#assign _page_id="group-detail">
<#assign _page_parent_name="회원 단체 관리">
<#assign _page_current_name="단체 수정">
...
<#-- row -->
    ...
    <#-- NEW WIDGET START -->
        ...
        <#-- Widget ID (each widget will need unique ID)-->
            ...
            <@am.widget_title icon="fa-eye">단체 수정</@am.widget_title>
                            ...
                            <div class="row">
                                <div class="smart-form">
                                    <label class="input" style="padding-left:10px;padding-right:10px;">
                                        <input id="document_title" placeholder="단체명을 입력하세요(최대 64자)" value="${documentEntity.document_title?html}" />
                                    </label>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    <#-- 게시판 관리로 이동 -->
        ...
        <@am.jsevent_prevent />
            my.fn.pmove('#${Request.contextPath}/admin/group/list');
        });
    ...
    <#-- 게시물 삭제 -->
        ...
        <@am.jsevent_prevent />
            ...
            method="DELETE" errortitle="단체 삭제 실패" ; job >
                <#if job == "success_job">
                <#-- 삭제 성공시 리스트 페이지로 이동 -->
                    <@am.jsnoti title="단체 삭제 완료" content="successText" boxtype="success" />
                ...
                <#elseif job == "common_job">
                    $this.removeClass('disabled').html('<i class="fa fa-trash-o"></i> 단체 삭제');
                </#if>
            </@am.jsajax_request>
            };

            my.fn.showUserConfirm('단체',
                    "단체를 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                    '삭제를 입력하세요',
                    '네', '삭제', process, '단체 삭제 실패', '삭제를 입력하지 않았습니다.');
        });
    ...
    <#-- 게시물 수정 -->
        ...
        <@am.jsevent_prevent />
            ...
            if($.trim(docTitle) == '') {
            <@am.jsnoti title="단체 수정 실패" content="단체명을 입력 하세요." contenttype="text" />
                return;
            }
        ...
        moveuri="#${Request.contextPath}/admin/group/list" errortitle="단체 수정 실패" ; job >
            <#if job == "success_job">
            <#-- 게시믈 수정 성공하면 toast 메시지를 보여 준다 -->
                var toastMsg = '단체를 수정 하였습니다.';
                <@am.jsnoti title="단체 수정 성공" content="toastMsg" boxtype="success" />

            <#elseif job == "common_job">
            <#-- 공통 작업. disabled 시킨 버튼을 enabled 시킨다 -->
                $btnAdd.removeClass('disabled').html('<i class="fa fa-check"></i> 단체 수정');
            </#if>
        </@am.jsajax_request>
        });
