...
<!------------------------------------------------------------------------------------------->
<#assign _page_id="user-secret-list">
<#assign _page_parent_name="비밀공간">
<#assign _page_current_name="비밀공간 목록">
...
$(document).ready(function() {
    ...
    <@ap.py_jsdatatable listId="${_page_id}" searchTooltip="제목 첫글자 기준 검색" url="${Request.contextPath}/user/secret/list/t/" ; job>
        <#if job == "order">
                [[0, 'desc']]
        <#elseif job == "columns">
                [
                ...
                {sName: 'document_title', mData: 'document_title', sClass: 'text-center', bSortable: false, bSearchable: true,
                    ...
                    <#-- 클릭시 a link 로 detail 페이지로 이동 -->
                        var navHash = $(location).attr('hash').substring(1),
                                moveURL = '${Request.contextPath}/user/secret/' + full.document_srl ;
