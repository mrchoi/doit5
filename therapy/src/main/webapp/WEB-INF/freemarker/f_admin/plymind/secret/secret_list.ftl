...
<#assign _page_id="secret_list">
<#assign _page_parent_name="비밀게시판">
<#assign _page_current_name="비밀게시판 목록">

<@am.page_simple_depth icon="fa-edit" parent="${_page_parent_name}" current="${_page_current_name}" />

<#--<@am.page_desc title="비밀공간" id="${_page_id}-helper">
<p>
    “쉿! 아무도 당신이 누군지 몰라요”<br>
    마음속 깊이 묻어둔 이야기를 털어놓기로 해요.
</p>
</@am.page_desc>-->
...
<script type="text/javascript">
    ...
    <#-- 게시물 추가를 선택하면 추가 폼을 보여 준다. -->
        $('#add-document').click(function(e) {
            var moveURL = '#${Request.contextPath}/admin/secret/add';
        <@am.jschangecontent moveuri="moveURL" navuri="navHash" isVarMove=true isVarNav=true />
        });

    <@am.jsdatatable listId="${_page_id}" url="${Request.contextPath}/admin/secret/list/t/" ; job>
        <#if job == "buttons">
                [{
                    sButtonText: '게시물 삭제',
                    sExtends: 'text',
        ...
        <#elseif job == "columns">
                ...
                {sName: 'document_title', mData: 'document_title', bSortable: true, bSearchable: true,
                    ...
                    <#-- 클릭시 a link 로 detail 페이지로 이동 -->
                        var navHash = $(location).attr('hash').substring(1),
                                moveURL = '#${Request.contextPath}/admin/secret/' + full.document_srl + '/';
                ...
                {sName: 'read_count', mData: 'read_count', sClass: 'dt_column_text_center', bSortable: false, bSearchable: false},
