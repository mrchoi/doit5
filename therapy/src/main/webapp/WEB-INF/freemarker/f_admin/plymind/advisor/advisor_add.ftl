<#assign _page_id="add-advisor">
<#assign _page_parent_name="상담사 관리">
<#assign _page_current_name="상담사 추가">
...
<p>
    추가된 상담사는 <a href="#${Request.contextPath}/admin/advisor/list" title="상담사 목록">상담사 목록</a> 메뉴에서 확인 가능 합니다.
</p>
</@am.page_desc>
...
<#-- row -->
    ...
    <#-- NEW WIDGET START -->
        ...
        <#-- Widget ID (each widget will need unique ID)-->
            ...
            <@am.widget_title icon="fa-edit">상담사 추가</@am.widget_title>
                ...
                <#-- widget content -->
                    <div class="widget-body">
                        <form id="checkout-form">
                            <fieldset>
                                <legend>추가할 상담사 정보를 입력 합니다.</legend>
                                ...
                                <hr class="app-user-group-info" style="display: block;" />
                                <div class="row app-user-group-info" style="display: block;">

                                <@am.bstrap_in_txt name="certificate" value="" title="상담사 자격증명"
                                maxlength="64" placeholder="최대 64byte 까지 가능 합니다" />

                                <@am.bstrap_in_txt name="kakao_id" value="" title="카카오 아이디"
                                maxlength="128" placeholder="최대 128byte 까지 가능 합니다" />

                                <@am.bstrap_in_select name="class_srl" title="상담사 등급">
                                    <option value="1">으뜸</option>
                                    <option value="2">가장</option>
                                    <option value="3">한마루</option>
                                </@am.bstrap_in_select>

                                <@am.bstrap_in_select name="domain_srl" title="상담사 영역">
                                    <option value="1">일과 업무</option>
                                    <option value="2">삶</option>
                                    <option value="3">부모</option>
                                    <option value="4">진로</option>
                                    <option value="5">대인관계</option>
                                    <option value="6">학습과 시험</option>
                                    <option value="7">중독</option>
                                    <option value="8">분노와 충동성</option>
                                </@am.bstrap_in_select>

                                <@am.bstrap_in_txtarea name="self_introduction" value="" title="상담사 자기소개"
                                placeholder="최대 512byte 까지 가능 합니다" />
                                </div>
    ...
    <#-- 사용자를 등록 한다 -->
        ...
        <#-- request 할 데이터를 만든다.(reqData 라는 이름을 왠만하면 바꾸지 말자. 이름을 그대로 사용해야 macro 사용이 편해짐) -->
            ...
            var reqData = {
                ...
                app_account_type:   '4',
                app_srl:            '100',
                certificate:        $.trim($('input[name=certificate]').val()),
                class_srl:          $('select[name=class_srl]').val(),
                domain_srl:         $('select[name=domain_srl]').val(),
                kakao_id:           $.trim($('input[name=kakao_id]').val()),
                self_introduction:  $.trim($('textarea[name=self_introduction]').val()).replace(/\n/g,'<br>')
            };
        ...
        moveuri="#${Request.contextPath}/admin/advisor/list" errortitle="상담사 추가 실패" ; job >
            <#if job == "success_job">
            <#-- 사용자 추가 성공하면 toast 메시지를 보여 준다 -->
                var toastMsg = reqData.user_id + ' 상담사를 추가 하였습니다.';
            ...
            <#-- 공통 작업. disabled 시킨 버튼을 enabled 시킨다 -->
                $btnAdd.removeClass('disabled').html('<i class="fa fa-check"></i> 상담사 추가');
            </#if>
        </@am.jsajax_request>
        });
    };
