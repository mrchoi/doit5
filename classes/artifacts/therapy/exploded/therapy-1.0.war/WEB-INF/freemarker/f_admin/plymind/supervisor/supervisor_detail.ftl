<#assign _page_id="supervisor_detail">
<#assign _page_parent_name="슈퍼바이저 관리">
<#assign _page_current_name="슈퍼바이 수정">
...
<section id="widget-grid" class="">
    <div class="row">
        <article class="col-sm-12 col-md-12 col-lg-12">
            ...
            <@am.widget_title icon="fa-eye">슈퍼바이저 수정</@am.widget_title>
                <div>
                    <div class="widget-body">
                        <form>
                            <fieldset style="padding:10px;">
                                <div class="row">
                                    <@am.bstrap_in_txt name="document_title" value="${documentEntity.document_title?html}" title="성함" maxlength="64" placeholder="최대 64byte 까지 가능 합니다."/>
                                </div>
                        ...
                        <div class="form-actions">
                            <div class="btn-group">
                                <button id="btn-delete" class="btn btn-sm btn-danger" type="button">
                                    <i class="fa fa-trash-o"></i> 슈퍼바이저 삭제
                                </button>
                            </div>
                            <div class="btn-group">
                                <button id="btn-add" class="btn btn-sm btn-success" type="button">
                                    <i class="fa fa-check"></i> 슈퍼바이저 수정
                                </button>
                            </div>
...
<script type="text/javascript">
    ...
    var pagefunction = function() {
        ...
        <@am.jsevent_prevent />
            ...
            var successText = '슈퍼바이저를 삭제 했습니다.';
            ...
            method="DELETE" errortitle="슈퍼바이저 삭제 실패" ; job >
                ...
                <#elseif job == "common_job">
                    $this.removeClass('disabled').html('<i class="fa fa-trash-o"></i> 슈퍼바이저 삭제');
                </#if>
            </@am.jsajax_request>
            };

            my.fn.showUserConfirm('슈퍼바이저 삭제',
                    "슈퍼바이저를 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                    '삭제를 입력하세요',
    ...
    <#-- 슈퍼바이저 수정 -->
        ...
        moveuri="#${Request.contextPath}/admin/supervisor/list" errortitle="수정 실패" ; job >
            <#if job == "success_job">
            <#-- 슈퍼바이저 수정 성공하면 toast 메시지를 보여 준다 -->
                var toastMsg = '슈퍼바이저를 수정 하였습니다.';
                <@am.jsnoti title="수정 성공" content="toastMsg" boxtype="success" />

            <#elseif job == "common_job">
            <#-- 공통 작업. disabled 시킨 버튼을 enabled 시킨다 -->
                $btnAdd.removeClass('disabled').html('<i class="fa fa-check"></i> 슈퍼바이저 수정');
            </#if>
        </@am.jsajax_request>
        });
    };
