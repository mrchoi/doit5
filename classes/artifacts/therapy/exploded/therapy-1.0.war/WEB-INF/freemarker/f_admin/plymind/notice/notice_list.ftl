...
<@am.page_desc title="공지사항/이벤트" id="${_page_id}-helper">
<p>
                    ...
                    <#-- datatable 에서 검색을 지원하는 column 의 th 는 dt-column, dt-column-<column number> class 를 포함해야 한다.(localStorage를 이용한 초기화에 이용됨) -->
                        <table id="${_page_id}" class="table table-striped table-bordered" width="100%">
                            ...
                            <tr>
                                ...
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 70px;min-width: 70px;">구분</th>
                                ...
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 50px;min-width: 50px;" >댓글수</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 100px;min-width: 100px;" >차단여부</th>
                            </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
<script type="text/javascript">
    ...
    <#-- 게시물 추가를 선택하면 추가 폼을 보여 준다. -->
        $('#add-document').click(function(e) {
            var moveURL = '#${Request.contextPath}/admin/notice/add';
        <@am.jschangecontent moveuri="moveURL" navuri="navHash" isVarMove=true isVarNav=true />
        });

    <@am.jsdatatable listId="${_page_id}" url="${Request.contextPath}/admin/notice/list/t/" ; job>
        <#if job == "buttons">
                [{
                    sButtonText: '게시물 삭제',
                    sExtends: 'text',
                    ...
                    <#-- 선택 된 member_srl을 구한다 -->
                        ...
                        if(reqData.l_keys.length <= 0) {
                            <@am.jsnoti title="공지사항 삭제 실패" content="선택된 공지사항이 없습니다" contenttype="text" />
                            return;
                        }

                        var successText = reqData.l_keys.length+'개의 공지사항을 삭제 했습니다.';
                        ...
                        <#-- 선택 된 디바이스 삭제 요청 -->
                            ...
                            method="DELETE" errortitle="공지사항 삭제 실패" ; job >
                                <#if job == "success_job">
                        ...
                        my.fn.showUserConfirm('공지사항 삭제',
                                "선택된 공지사항을 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                                '삭제를 입력하세요',
        ...
        <#elseif job == "columns">
                ...
                {sName: 'allow_notice', mData: 'allow_notice', sClass: 'dt_column_text_center', bSortable: true, bSearchable: true,
                    mRender: function(data, type, full){
                        if(data == ${mdv_yes})      return '공지사항';
                        else if(data == ${mdv_no})  return '이벤트';
                        return '-';
                    }
                },
                {sName: 'document_title', mData: 'document_title', bSortable: true, bSearchable: true,
                    ...
                    <#-- 클릭시 a link 로 detail 페이지로 이동 -->
                        var navHash = $(location).attr('hash').substring(1),
                                moveURL = '#${Request.contextPath}/admin/notice/' + full.document_srl + '/';
                ...
                {sName: 'comment_count', mData: 'comment_count', sClass: 'dt_column_text_center', bSortable: false, bSearchable: false},
