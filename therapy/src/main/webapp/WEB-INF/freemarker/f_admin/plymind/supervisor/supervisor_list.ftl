<#assign _page_id="supervisor_list">
<#assign _page_parent_name="슈퍼바이저 관리">
<#assign _page_current_name="슈퍼바이저 목록">

<@am.page_simple_depth icon="fa-edit" parent="${_page_parent_name}" current="${_page_current_name}" />

<@am.page_desc title="슈퍼바이저" id="${_page_id}-helper">
<p>
    플리마인드에 계신 슈퍼바이저를 소개합니다.<br>

    플리마인드에는 임상심리학회, 상담삼리학회, 상담학회 슈퍼바이저 선생님들이 계십니다.<br>

    슈퍼바이저가 되시길 원하는 슈퍼바이저 수련선생님들에게 온라인 슈퍼비젼을 제공해 드립니다.<br>

    문의 주시면 자세히 알려드리겠습니다.<br>
                    ...
                    <#-- datatable 에서 검색을 지원하는 column 의 th 는 dt-column, dt-column-<column number> class 를 포함해야 한다.(localStorage를 이용한 초기화에 이용됨) -->
                        <table id="${_page_id}" class="table table-striped table-bordered" width="100%">
                            ...
                            <tr>
                                ...
                                <th class="hasinput">
                                    <input type="text" class="form-control input-xs dt-column dt-column-2" placeholder="성함 검색"
                                           rel="tooltip" data-placement="top" data-original-title="성함 기준 검색" />
                                </th>
                                <th class="hasinput"></th>
                                <th class="hasinput"></th>
                                <th class="hasinput"></th>
                            ...
                            <tr>
                                ...
                                <th data-class="expand" style="text-align: center; max-width: 100px;min-width: 100px">성함</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 200px;min-width: 200px;" >활동영역(자격증번호)</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 200px;min-width: 200px;" >근무처(직위)</th>
...
<script type="text/javascript">
    ...
    <#-- 게시물 추가를 선택하면 추가 폼을 보여 준다. -->
        $('#add-document').click(function(e) {
            var moveURL = '#${Request.contextPath}/admin/supervisor/add';
        <@am.jschangecontent moveuri="moveURL" navuri="navHash" isVarMove=true isVarNav=true />
        });

    <@am.jsdatatable listId="${_page_id}" url="${Request.contextPath}/admin/supervisor/list/t/" ; job>
        <#if job == "buttons">
                [{
                    sButtonText: '슈퍼바이저 삭제',
                    sExtends: 'text',
                    ...
                    <#-- 선택 된 member_srl을 구한다 -->
                        ...
                        if(reqData.l_keys.length <= 0) {
                            <@am.jsnoti title="슈퍼바이저 삭제 실패" content="선택된 슈퍼바이저가 없습니다" contenttype="text" />
                            return;
                        }

                        var successText = reqData.l_keys.length+'개의 슈퍼바이저를 삭제 했습니다.';
                        ...
                        <#-- 선택 된 디바이스 삭제 요청 -->
                            ...
                            method="DELETE" errortitle="슈퍼바이저 삭제 실패" ; job >
                                <#if job == "success_job">
                                <#-- 삭제 성공시 테이블을 다시 로딩. remove 하면 내부 데이터만 삭제되지 화면에 반영 되지 않는다.
                                     draw 를 호출 해야 반영 된다. -->
                                    <@am.jsnoti title="슈퍼바이저 삭제 완료" content="successText" boxtype="success" />
                                ...
                                <#elseif job == "common_job">
                                    $.unblockUI();
                                    $btn.removeClass('disabled').html('<span>사용자 삭제</span>');
                                </#if>
                            </@am.jsajax_request>
                        };

                        my.fn.showUserConfirm('슈퍼바이저 삭제',
                                "선택된 슈퍼바이저를 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                                '삭제를 입력하세요',
                                '네', '삭제', process, '슈퍼바이저 삭제 실패', '삭제를 입력하지 않았습니다.');
                    }
                }]
        <#elseif job == "order">
                [[1, 'desc']]
        <#elseif job == "columns">
                ...
                {sName: 'document_title', mData: 'document_title', sClass: 'dt_column_text_center', bSortable: true, bSearchable: true,
                    mRender: function(data, type, full, dtObj){
                    <#-- 클릭시 a link 로 detail 페이지로 이동 -->
                        var navHash = $(location).attr('hash').substring(1),
                                moveURL = '#${Request.contextPath}/admin/supervisor/' + full.document_srl + '/';
                        return '<a class="datatable-row-detail-view" data-move="' +
                                moveURL + '" data-nav="' + navHash + '">'+data+'</a>';
                    }
                },
                {sName: 'template_extra_work', mData: 'template_extra.work', sClass: 'dt_column_text_center', bSortable: false, bSearchable: false},
                {sName: 'template_extra_office', mData: 'template_extra.office', sClass: 'dt_column_text_center', bSortable: false, bSearchable: false},
