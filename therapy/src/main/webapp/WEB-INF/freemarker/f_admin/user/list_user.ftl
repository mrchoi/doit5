...
<#assign _page_id="list-user">
<#assign _page_parent_name="사용자">
<#assign _page_current_name="사용자 목록">
...
<p>
    플리마인드에 가입된 관리자 및 사용자 리스트를 보여 줍니다.
<p>
    관리자 및 사용자 삭제를 위해서는 열의 제일 앞에 있는 선택을 표시하고 <strong>사용자 삭제</strong> 버튼을 누르면 선택된 사용자가
    <span class="text-success help-cursor" rel="tooltip" data-placement="top" data-original-title="사용자 삭제 버튼"><strong>삭제</strong></span> 됩니다.
</p>
</@am.page_desc>
    ...
    <div class="row">
        ...
        <article class="col-sm-12 col-md-12 col-lg-12">
            ...
            <div class="jarviswidget" id="wid-list-app-0" data-widget-colorbutton="false" data-widget-editbutton="false"
                ...
                <@am.widget_title icon="fa-table">사용자 목록</@am.widget_title>
                    ...
                    <div class="widget-body no-padding">
                        ...
                        <table id="${_page_id}" class="table table-striped table-bordered" width="100%">
                            ...
                            <tr>
                                ...
                                <th class="hasinput"></th>
                                <th class="hasinput">
                                    <input type="text" class="form-control input-xs dt-column dt-column-3" placeholder="사용자 아이디 검색"
                            ...
                            <tr>
                                ...
                                <th style="text-align: center; max-width: 45px;min-width: 45px;">계정</th>
                                <th style="text-align: center;">아이디</th>
                                <th data-hide="phone" style="text-align: center; max-width: 228px;">이름</th>
...
<script type="text/javascript">
    ...
    var pagefunction = function() {
        ...
        <@am.jsdatatable listId="${_page_id}" url="${Request.contextPath}/admin/member/list/t/" ; job>
            <#if job == "buttons">
                [{
                    sButtonText: '사용자 삭제',
                    sExtends: 'text',
                    fnClick: function(nButton, oConfig, oFlash) {
                        ...
                        if(reqData.l_keys.length <= 0) {
                            <@am.jsnoti title="사용자 삭제 실패" content="선택된 사용자가 없습니다" contenttype="text" />
                            return;
                        }

                        var successText = reqData.l_keys.length+'명의 사용자를 삭제 했습니다.';
                        var process = function() {
                            ...
                            <@am.jsajax_request submituri="${Request.contextPath}/admin/member/t/"
                                    method="DELETE" errortitle="사용자 삭제 실패" ; job >
                                <#if job == "success_job">
                                    <#-- 삭제 성공시 테이블을 다시 로딩. remove 하면 내부 데이터만 삭제되지 화면에 반영 되지 않는다.
                                         draw 를 호출 해야 반영 된다. -->
                                    <@am.jsnoti title="사용자 삭제 완료" content="successText" boxtype="success" />
                        ...
                        my.fn.showUserConfirm('사용자 삭제',
                                "선택된 사용자를 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                                '삭제를 입력하세요',
                                '네', '삭제', process, '사용자 삭제 실패', '삭제를 입력하지 않았습니다.');
                    }
                }]
            <#elseif job == "order">
                [[1, 'desc']]
            <#elseif job == "columns">
                [
                    ...
                    {sName: 'account_type', mData: null, bSortable: false, bSearchable: false,
                        mRender: function(data, type, full, dtObj) {
                            var member_group_srl = "";
                            for(var i=0; i<full.account_type.length; i++) {
                                if(full.account_type[i].group_srl == ${backoffice_root}) member_group_srl += "관리자 ";
                                else if(full.account_type[i].group_srl == ${backoffice_user}) member_group_srl += "사용자 ";
                            }

                            return member_group_srl;

                            /*if (data == '${backoffice_root}')      return '관리자';
                            else if (data == '${backoffice_user}')  return '사용자';
                            return data;*/
                        }
                    },
                    {sName: 'user_id', mData: 'user_id', bSortable: true, bSearchable: true,
                        mRender: function(data, type, full, dtObj){
                            ...
                            var navHash = $(location).attr('hash').substring(1),
                                    moveURL = '#${Request.contextPath}/admin/member/' + full.member_srl;
