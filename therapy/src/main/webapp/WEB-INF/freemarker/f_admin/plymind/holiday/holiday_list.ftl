<#assign _page_id="holiday-holiday-list">
<#assign _page_parent_name="플리마인드 관리">
<#assign _page_current_name="캘린더 휴일 관리">
    ...
    <div class="row">
        <article class="col-sm-12 col-md-12 col-lg-12">
            <div class="jarviswidget" id="wid-list-app-0" data-widget-colorbutton="false" data-widget-editbutton="false"
                ...
                <@am.widget_title icon="fa-table">휴일 목록</@am.widget_title>
                <div>
                    <div class="widget-body no-padding">
                        <table id="${_page_id}" class="table table-striped table-bordered" width="100%">
                            ...
                            <tr>
                                ...
                                <th class="hasinput">
                                    <input type="text" class="form-control input-xs dt-column dt-column-2" placeholder="휴일명 검색"
                                           rel="tooltip" data-placement="top" data-original-title="휴일명 기준 검색" />
                                </th>
                                <th class="hasinput"></th>
                                <th class="hasinput"></th>
                            </tr>
...
<script type="text/javascript">
    ...
    <#-- 게시물 추가를 선택하면 추가 폼을 보여 준다. -->
        $('#add_holiday').click(function(e) {
            var moveURL = '#${Request.contextPath}/admin/holiday/holiday_add';
            <@am.jschangecontent moveuri="moveURL" navuri="navHash" isVarMove=true isVarNav=true />
        });

    <@am.jsdatatable listId="${_page_id}" url="${Request.contextPath}/admin/holiday/holiday_list/t/" ; job>
        <#if job == "buttons">
                [{
                    sButtonText: '휴일 삭제',
                    sExtends: 'text',
                    ...
                    <#-- 선택 된 member_srl을 구한다 -->
                        ...
                        if(reqData.l_keys.length <= 0) {
                            <@am.jsnoti title="휴일 삭제 실패" content="선택된 휴일이 없습니다" contenttype="text" />
                            return;
                        }
                        var successText = reqData.l_keys.length+'개의 휴일을 삭제 했습니다.';
                        ...
                        <#-- 화면 전체 block 을 띄운다 -->
                            ...
                            method="DELETE" errortitle="휴일 삭제 실패" ; job >
                                <#if job == "success_job">
                                    <@am.jsnoti title="휴일 삭제 완료" content="successText" boxtype="success" />
                        ...
                        my.fn.showUserConfirm('휴일 삭제',
                                "선택된 휴일을 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                                '삭제를 입력하세요',
        ...
        <#elseif job == "columns">
            [
                ...
                {sName: 'holiday_date', mData: 'holiday_date', sClass: 'text-center', bSortable: false, bSearchable: false,
                    mRender: function(data, type, full){
