...
<#assign _page_id="group-add">
<#assign _page_parent_name="회원 단체 관리">
<#assign _page_current_name="단체 등록">
...
<#-- row -->
    ...
    <#-- NEW WIDGET START -->
        ...
        <#-- Widget ID (each widget will need unique ID)-->
            ...
            <@am.widget_title icon="fa-edit">단체 추가</@am.widget_title>
                            ...
                            <div class="row">
                                <div class="smart-form">
                                    <label class="input" style="padding-left:10px;padding-right:10px;">
                                        <input id="document_title" placeholder="단체명을 입력하세요(최대 64자)" />
                                    </label>
                                </div>
                            </div>
                        </div>
...
<script type="text/javascript">
    ...
    <#-- 게시물 추가 -->
        ...
        <@am.jsevent_prevent />
            ...
            if($.trim(docTitle) == '') {
            <@am.jsnoti title="단체 등록 실패" content="단체명을 입력 하세요." contenttype="text" />
                return;
            }
        ...
        moveuri="#${Request.contextPath}/admin/group/list" errortitle="단체 등록 실패" ; job >
            <#if job == "success_job">
            <#-- 게시믈 등록 성공하면 toast 메시지를 보여 준다 -->
                var toastMsg = '단체를 등록 하였습니다.';
                <@am.jsnoti title="단체 등록 성공" content="toastMsg" boxtype="success" />
            <#-- 게시물 등록 후 테이블의 1페이지를 보여 주기 위해서 테이블의 localStorage 를 삭제 시킨다. -->
                my.fn.removeDatatableStorage('group_list');
            <#elseif job == "common_job">
            <#-- 공통 작업. disabled 시킨 버튼을 enabled 시킨다 -->
                $btnAdd.removeClass('disabled').html('<i class="fa fa-check"></i> 단체 등록');
            </#if>
        </@am.jsajax_request>
        });
    };
