<#assign _page_id="one-list">
<#assign _page_parent_name="1:1 문의 게시판">
<#assign _page_current_name="1:1 문의 게시판 목록">
                    ...
                    <#-- datatable 에서 검색을 지원하는 column 의 th 는 dt-column, dt-column-<column number> class 를 포함해야 한다.(localStorage를 이용한 초기화에 이용됨) -->
                        <table id="${_page_id}" class="table table-striped table-bordered" width="100%">
                            ...
                            <tr>
                                ...
                                <th class="hasinput">
                                    <input type="text" class="form-control input-xs dt-column dt-column-2" placeholder="제목 검색"
                                           rel="tooltip" data-placement="top" data-original-title="제목 기준 검색" />
                                </th>
                                <th class="hasinput"></th>
                                <th class="hasinput"></th>
                                <th class="hasinput">
                                    <#--<input type="text" class="form-control input-xs dt-column dt-column-5" placeholder="상담자 검색"
                                           rel="tooltip" data-placement="top" data-original-title="상담자 검색" />-->
                                </th>
                                <th class="hasinput"></th>
                                <th class="hasinput"></th>
                            </tr>
                            <tr>
                                ...
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 100px;min-width: 100px;" >작성자</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 150px;min-width: 150px;" >문의일</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 100px;min-width: 100px;" >상담사</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 100px;min-width: 100px;" >답변자</th>
                                <th data-hide="phone,tablet" style="text-align: center; max-width: 150px;min-width: 150px;" >답변일</th>
                            </tr>
                            </thead>
                            <tbody></tbody>
                        </table>
...
<script type="text/javascript">
    ...
    <#-- 게시물 추가를 선택하면 추가 폼을 보여 준다. -->
        $('#add-document').click(function(e) {
            var moveURL = '#${Request.contextPath}/admin/one/add';
        <@am.jschangecontent moveuri="moveURL" navuri="navHash" isVarMove=true isVarNav=true />
        });

    <@am.jsdatatable listId="${_page_id}" url="${Request.contextPath}/admin/one/list/t/" ; job>
        <#if job == "buttons">
                [{
                    sButtonText: '게시물 삭제',
                    sExtends: 'text',
                    ...
                    <#-- 선택 된 member_srl을 구한다 -->
                        ...
                        if(reqData.l_keys.length <= 0) {
                            <@am.jsnoti title="게시물 삭제 실패" content="선택된 게시물이 없습니다" contenttype="text" />
                            return;
                        }
                        ...
                        <#-- 선택 된 디바이스 삭제 요청 -->
                            ...
                            method="DELETE" errortitle="게시물 삭제 실패" ; job >
                                ...
                                <#elseif job == "common_job">
                                    $.unblockUI();
                                    $btn.removeClass('disabled').html('<span>게시물 삭제</span>');
                                </#if>
                            </@am.jsajax_request>
                        };

                        my.fn.showUserConfirm('게시물 삭제',
                                "선택된 게시물을 삭제 합니다.<br/>삭제를 원하면 \'삭제\'를 입력 후 \'네\' 버튼을 클릭 하세요.",
                                '삭제를 입력하세요',
        ...
        <#elseif job == "columns">
                ...
                {sName: 'document_title', mData: 'document_title', bSortable: true, bSearchable: true,
                    ...
                    <#-- 클릭시 a link 로 detail 페이지로 이동 -->
                        var navHash = $(location).attr('hash').substring(1),
                                moveURL = '#${Request.contextPath}/admin/one/' + full.document_srl + '/';
                ...
                {sName: 'advisor_srl', mData: 'advisor_srl', sClass: 'text-center', bSortable: false, bSearchable: false,
                    mRender: function(data, type, full){
                        if(data != "") return full.advisor_nick_name;
                        return '-';
                    }
                },
                {sName: 'comment_user_id', mData: 'comment_user_id', sClass: 'text-center', bSortable: false, bSearchable: false},
